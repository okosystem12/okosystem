...
def confirm(request):
    ...
    if request.POST:
        ...
        if _user.exists():
            _c = VideoCorrupt.objects.filter(Q(pk=_id) & Q(video__social__controlUser__in=_user)).first()
            if _c:
                _c.confirmedAt = datetime.now()
                _c.save()

                args.update(success(_user))
                log(request.user.pk, 'Данные КП', 'Управление', 'Подтверждение Видеозапись')
        else:
