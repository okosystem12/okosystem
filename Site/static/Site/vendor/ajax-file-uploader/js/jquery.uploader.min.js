(() => {
    ...
    class h {
        ...
        initEle() {
            ...
            if (this.$originEle.css("display", "none"), this.$selectCard = t()(
                !this.options.gallery
                    ? ('<div class="jquery-uploader-select-card ' + (this.options.multiple ? "" : "jquery-uploader-select-card-solo") + '">                ' +
                ...
                this.$uploader = t()(`<div class="jquery-uploader ${(this.options.gallery ? "jquery-uploader-gallery" : "")}"></div>`).append(this.$uploaderContainer),
                this.options.parent ? this.options.parent.append(this.$uploader) : this.$originEle.parent().append(this.$uploader), this.options.mode === h.mode.url) {
        ...
        createFileCardEle(e, i, r) {
            ...
            let l = (r = r || p(i)) === a ? `<img alt="preview" class="files_img" src="${i}"/>` : '<div class="file_other"></div>',
                o = t()(`<div class="jquery-uploader-card ${this.options.multiple ? "" : "jquery-uploader-card-solo"}" id="${e}">                    
<div class="jquery-uploader-preview-main">                       
<div class="jquery-uploader-preview-action">                  
 <ul>
 ${this.options.gallery
                    ? '<li class="file-detail"><i class="glyphicon glyphicon-eye-open"></i></li>'
                    : '<li class="file-delete"><i class="glyphicon glyphicon-trash"></i></li>'}                   
             </ul>                        </div>                 
...
${l}                    </div>                 </div>`);
        ...
        refreshPreviewFileList() {
            ...
            this.$uploaderContainer.empty(), this.files.forEach((e => {
                this.$uploaderContainer.append(e.$ele),
                    e.$ele.find(".jquery-uploader-preview-action .file-delete").on("click", this.handleFileDelete.bind(this))
                e.$ele.find(".jquery-uploader-preview-action .file-detail").on("click", this.handleFileDetail.bind(this))
        ...
        createDefaultFiles(defaultValue) {
            defaultValue && defaultValue.forEach((e, i) => {
                let id = u(), r = p(e.url), t = this.createFileCardEle(id, e.url, r);
                this.files.push({
                    id,
                    type: r,
                    name: e.name || 'file' + i,
        ...
        handleFileSelect() {
            ...
            t()(`<input type="file" ${this.options.multiple ? "multiple" : ""}  ${this.options.accept ? `accept="${this.options.accept}"` : ""}  />`).on("change", (e => {
    ...
    h.config = {
        ...
        $.fn.uploaderClean = function () {
            let i = $(this), t = i.data(r);
            t && t.clean();
        },
        $.fn.uploaderDefaultFiles = function (defaultFiles) {
            let i = $(this), t = i.data(r);
            t && t.createDefaultFiles(defaultFiles);
        }
        $.fn.uploaderFileList = function () {
            let i = $(this), t = i.data(r);
            return t ? t.files : [];
        }
})();
